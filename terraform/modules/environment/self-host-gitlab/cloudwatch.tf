...
resource "aws_cloudwatch_event_rule" "start_gitlab_rule" {
  ...
  name                = "${var.pj}-GitLab-StartRule"
  description         = "Start ${var.pj} GitLab"
  ...
  input = <<DOC
{
  ...
  "AutomationAssumeRole": ["${aws_iam_role.gitlab_ssm_automation.0.arn}"]
}
DOC
}
...
resource "aws_cloudwatch_event_rule" "stop_gitlab_rule" {
  ...
  name                = "${var.pj}-GitLab-StopRule"
  description         = "Stop ${var.pj} GitLab"
